...
crop <- readRDS("./Objects/Domain_BS.rds") %>%  # Load SF polygons of the MiMeMo model domains
  ...
  st_transform(crs = 4326) %>% #convert back incase it needs to be in 4326 for some reason
...
skip <- list.files("./Objects/NE_Days/Barents_Sea")                                      # Already processed files
...
all_files %>%
  ...
  future_map(NEMO_ERSEM, analysis = "slabR", summary = scheme_result,
             ...
             out_dir = "./Objects/NE_Days/Barents_Sea", 
...
#### check ####
# 
# NE.01.2016 <- readRDS("./Objects/NE_Days/Barents_Sea/NE.GFDL.ssp126.01.2015.rds") %>%
