...
packages <- c("tidyverse", "nemoRsem", "furrr", "ncdf4","sf")                                 # List packages
lapply(packages, library, character.only = TRUE)                            # Load packages
source("./@_Region_file_EG.R")   # Define project region 
...
domains <- readRDS("./Objects/Domain_EG.rds") %>%                             # Load SF polygons of the MiMeMo model domains
  dplyr::select(-c(Elevation, area))                                               # Drop unneeded data which would get included in new NM files

crop <- readRDS("./Objects/Domain_EG.rds") %>%  # Load SF polygons of the MiMeMo model domains
...
example <- filter(all_files, Type == "N3") %>% 
  slice(1) 
...
skip <- list.files("./Objects/NE_Months/East_Greenland/")                                      # Already processed files
...
all_files %>%
  ...
  future_map(NEMO_ERSEM, analysis = "slabR", summary = scheme_result,
             ...
             out_dir = "./Objects/NE_Months/East_Greenland", 
...
#### check ####
# NE.01.2017 <- readRDS("./Objects/NE_Months/Barents_Sea/NE.01.2017.rds")
# 
# ggplot(NE.01.2016) +
