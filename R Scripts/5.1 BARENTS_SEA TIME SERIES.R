...
Months <- list.files("./Objects/NE_Months/Barents_Sea/", full.names = T) %>%                # Get list of NEMO-MEDUSA files
...
TS <- list.files("./Objects/NE_Days/Barents_Sea/", full.names = T) %>%                      # Get list of NEMO-MEDUSA files
  future_map(NE_volume_summary,                                                 # Read in the months and calculate mean compartments
             ice = FALSE,
             .progress = T) %>%      
  data.table::rbindlist() %>%                                                   # Combine timesteps into series
...
saveRDS(TS, "./Objects/TS_Barents_Sea.rds")                                                 # Save out time series in the folder above
toc()                                                                           # Stop timing
