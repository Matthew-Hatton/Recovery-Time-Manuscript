...
Months <- list.files("./Objects/NE_Months/Barents_Sea/", full.names = T) %>%            # Get list of NE files
...
Days <- list.files("./Objects/NE_Days/Barents_Sea/", full.names = T) %>%                # Get list of NE files
  future_map(decadal, .progress = TRUE) %>%                                 # Read in data, remove unnecessary columns and create a decade column
...
saveRDS(SP, "./Objects/SPATIAL_Barents_Sea.rds")                                          # Save out spatial file
